...
class MeshSegmentation:
    ...
    def calc_geodesic_dist(self):
        adj_faces_v = self.mesh.faces[self.adj_faces]
        # mesh.vertices[adj_faces_v] has shape #faces x 3 (verts) x 3 (dim)
        faces_centers = self.mesh.vertices[adj_faces_v].mean(axis=1)
        edges_centers = self.mesh.vertices[self.shared_edges].mean(axis=1)
