...
class DGCNN(BaseModel):
    N_OUTPUT = 512
    def __init__(self, global_embedding=False, **basemodel_kwargs):
        ...
        self.bn8 = nn.BatchNorm1d(1024)  # 256
        self.bn9 = nn.BatchNorm1d(512)  # 256
        ...
        #                           self.bn7,
        #                           nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(1216, 1024, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.dp1 = nn.Dropout(p=0.5)
        self.conv9 = nn.Sequential(nn.Conv1d(1024, 512, kernel_size=1, bias=False),
                                   self.bn9,
                                   nn.LeakyReLU(negative_slope=0.2))
        ...
        # self.conv11 = nn.Conv1d(128, self.n_classes, kernel_size=1, bias=False)
    ...
    def forward_features(self, x):
        ...
        if self.global_embedding:
            return torch.cat([x_max, x_mean], dim=1).squeeze()'''
