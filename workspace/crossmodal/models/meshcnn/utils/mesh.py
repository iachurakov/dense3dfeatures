...
class Mesh:
    ...
    def __init__(self, data, hold_history=False, export_folder=''):
        attrs = ('vs', 'filename', 'features', 'edge_areas', 've', 'v_mask',
                 'edges', 'gemm_edges', 'sides', 'edges_count')
        for key in attrs:
            setattr(self, key, data[key])
            # if key == 'filename':
            #     setattr(self, key, data[key].asstr()[0])
            # elif key == 'edges_count':
            #     setattr(self, key, int(data[key][0]))
            # else:
            #     setattr(self, key, data[key][:])
