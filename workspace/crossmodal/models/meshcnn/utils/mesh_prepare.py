...
def from_scratch(file, opt, is_train):
    mesh_data = {'vs': None, 'edges': None, 'gemm_edges': None, 'sides': None, 'edges_count': None, 've': None,
                 'v_mask': None, 'filename': 'unknown', 'edge_lengths': None, 'edge_areas': []}

    mesh_data['vs'], faces = fill_from_file(mesh_data, file, opt)
    mesh_data['v_mask'] = np.ones(len(mesh_data['vs']), dtype=bool)

    faces, face_areas = remove_non_manifolds(mesh_data, faces)
    if is_train and opt.num_aug > 0:
...
def fill_from_file(mesh, file, opt):
    mesh['filename'] = 'PLACEHOLDER' #!  ntpath.split(file)[1]
    mesh['fullfilename'] = 'PLACEHOLDAER' #! file
    #! model = np.load(file)
    vs = file[0] #! model['vs']
    if opt.normalize:
        vs = normalize_mesh(vs)
    faces = file[1] #! model['faces'].astype(np.int64)
    return vs, faces


def normalize_mesh(x):
    """
    transforms mesh vertices coordinates to lie in [0, 1]
    """
    # vs -= vs.min(axis=0)
    # scale = vs.max()
    # return vs / scale
...
def compute_face_normals_and_areas(mesh, faces):
    ...
    # face_areas = np.where(face_areas == 0, 0.0001, face_areas)###
